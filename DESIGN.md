...
| **OCR compare** | User pastes an image into the card (`⌘V` — paste events need zero permissions, works for screenshots of any source: other tabs, PDFs, emails, phone photos via Universal Clipboard). Tesseract.js (bundled WASM, offscreen doc) extracts text locally; we scan for substrings matching the selected format and compare. (A `captureVisibleTab` screen-region scan was removed: it could only capture the same tab the field was on, which is useless — snip-and-paste covers every real source.) | On (paid tier) |
