...
| **Voice input** | User reads the source aloud; on-device SpeechRecognition (`processLocally: true`, Chrome 139+ only) transcribes in a hidden extension-origin iframe (mic permission belongs to the extension, not the page; transcripts return via runtime messaging the page can't see); digit words are normalized and run through the same candidate extraction as OCR. Never cloud recognition — if on-device is unavailable the button disables itself. (Implemented in v1 — by June 2026, Chrome 139 is a fair baseline.) | Per-card "Speak it" button, click-to-use |
