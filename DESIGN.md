...
| **Read-aloud (TTS)** | Page `speechSynthesis` from the content script, **local voices only** (`localService` filtered), digit-by-digit with grouping pauses. (Changed from `chrome.tts` during implementation: the value then never leaves the content script and the `tts` permission is dropped.) | **Off** (open-plan offices; user enables per session or in settings) |
...
| **Voice read-back** (v2) | User reads the source aloud; on-device SpeechRecognition (`processLocally: true`, Chrome 139+ only) transcribes; compare. Never cloud recognition — if on-device is unavailable, the feature is hidden, not degraded to Google's servers. | Off, v2 |
