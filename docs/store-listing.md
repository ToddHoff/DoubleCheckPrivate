...
Double Check installs with no access to any website and no standing permission to read pages — it opens only when you invoke it, and it never asks to be allowed onto a site. Verified values are never transmitted, logged, or stored. OCR and speech recognition run entirely on your device. There are no analytics and no telemetry. The only network traffic is license verification with our payment provider.
...
declared host. The extension declares no other host permissions — optional
or otherwise — so it never requests access to any website you visit.
